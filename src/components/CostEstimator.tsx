...
const COST_PER_IMAGE_GENERATION = 0.018; // Approx cost for a 1024x1024 image
...
export const CostEstimator: React.FC<CostEstimatorProps> = ({
  ...
  const {totalCost, styleAnalysisCost, storyboardCost} = useMemo(() => {
    ...
    // 2. Cost of generating the storyboard images and master prompt
    if (nonEmptyScenes.length > 0) {
      // Cost of generating the visual storyboard images
      if (generateVisuals) {
        currentStoryboardCost +=
          nonEmptyScenes.length * COST_PER_IMAGE_GENERATION;
      }

      // Cost of generating the master prompt (which takes text and optionally images as input)
      const totalSceneChars = nonEmptyScenes.reduce(
        (acc, scene) => acc + scene.description.length,
        0,
      );
      ...
      // Input images cost (the generated storyboard images are used as input for the master prompt)
      if (generateVisuals) {
        const imageCount = nonEmptyScenes.length;
        currentStoryboardCost += imageCount * COST_PER_IMAGE_INPUT;
      }

      // Output text cost (the master prompt)
      const totalShots = nonEmptyScenes.reduce(
        (acc, scene) => acc + scene.shotCount,
        0,
      );
      currentStoryboardCost += totalShots * 500 * COST_PER_OUTPUT_CHAR; // Assume ~500 char output per shot
    }
  ...
  }, [imageFile, scenes, generateVisuals]);
